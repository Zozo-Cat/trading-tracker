...
export default function NewsListWidget({ instanceId }: { instanceId: string }) {
    const [loading, setLoading] = useState(true);
    const [err, setErr] = useState<string | null>(null);
    const [items, setItems] = useState<ApiItem[]>([]);

    // Brugerens lokale tidszone (hydration-safe, hentes på klienten)
    const tz = useMemo(
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const url = `/api/calendar/next?limit=5&minImpact=1&hours=168&tz=${encodeURIComponent(
                    tz
                )}`;
...
function Row({ item, tz }: { item: ApiItem; tz: string }) {
    ...
    const dateStr = fmtLocal(dt, tz); // dd.MM, HH.mm
